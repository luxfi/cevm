...
//
// Bumped to 5 for v0.26.x: the C++ side added gpu_execute_block_v3 with a
// CBlockContext parameter and started populating BlockResult.status[] from
// the kernel paths. V2 callers continue to work — the V2 entry point
// internally treats BlockContext as zero. Callers that want CHAINID,
// TIMESTAMP, etc. honoured on the GPU MUST switch to ExecuteBlockV3.
